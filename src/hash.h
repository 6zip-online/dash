...
template <typename T>
Span<const unsigned char> MakeUCharSpan(T begin, T end) {
    return Span<const unsigned char>(reinterpret_cast<const unsigned char*>(&*begin), reinterpret_cast<const unsigned char*>(&*end));
}

template<typename T1, typename T2>
inline uint256 Hash(T1 begin1, T1 end1, T2 ptr1, T2 ptr2) {
    uint256 result;
    CHash256().Write(MakeUCharSpan(begin1, end1)).Write(MakeUCharSpan(ptr1, ptr2)).Finalize(result);
    return result;
}
