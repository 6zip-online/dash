...
FUZZ_TARGET(block_header)
{
    ...
    {
        const uint256 hash = block_header->GetPOWHash();
    ...
    }
    {
        ...
        assert(block.GetBlockHeader().GetPOWHash() == block_header->GetPOWHash());
        (void)block.ToString();
        block.SetNull();
        assert(block.GetBlockHeader().GetPOWHash() == mut_block_header.GetPOWHash());
    }
    {
