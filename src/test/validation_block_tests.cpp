...
struct TestSubscriber final : public CValidationInterface {
    ...
    void BlockConnected(const std::shared_ptr<const CBlock>& block, const CBlockIndex* pindex) override
    {
        ...
        m_expected_tip = block->GetPOWHash();
    }

    void BlockDisconnected(const std::shared_ptr<const CBlock>& block, const CBlockIndex* pindex) override
    {
        BOOST_CHECK_EQUAL(m_expected_tip, block->GetPOWHash());
...
BOOST_AUTO_TEST_CASE(processnewblock_signals_ordering)
{
    ...
    while (blocks.size() < 50) {
        blocks.clear();
        BuildChain(Params().GenesisBlock().GetPOWHash(), 100, 15, 10, 500, blocks);
    }
...
BOOST_AUTO_TEST_CASE(mempool_locks_reorg)
{
    ...
    auto last_mined = GoodBlock(Params().GenesisBlock().GetPOWHash());
    BOOST_REQUIRE(ProcessBlock(last_mined));

    // Run the test multiple times
    for (int test_runs = 3; test_runs > 0; --test_runs) {
        BOOST_CHECK_EQUAL(last_mined->GetPOWHash(), ::ChainActive().Tip()->GetBlockHash());
        ...
        for (int num_txs = 22; num_txs > 0; --num_txs) {
            ...
            last_mined = GoodBlock(last_mined->GetPOWHash());
            BOOST_REQUIRE(ProcessBlock(last_mined));
        }
        ...
        const uint256 tip_init{last_mined->GetPOWHash()};
