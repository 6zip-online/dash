...
const char * const BITCOIN_CONF_FILENAME = "zip.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //zip-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\ZipCore
    // macOS: ~/Library/Application Support/ZipCore
    // Unix-like: ~/.zipcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZipCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/ZipCore";
#else
    // Unix-like
    return pathRet / ".zipcore";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty zip.conf if it does not exist
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    // Check for untranslated substitution to make sure Zip Core copyright is not removed by accident
    if (copyright_devs.find("6Zip Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2024, nEndYear) + "The Zip Core developers";
    }
    // Check for untranslated substitution to make sure Zip Core copyright is not removed by accident
